...
（48 张封顶，每回合抽 5 张，天机 3/3，手牌上限 10 不可改），点手牌→放大 20%+轻摆→点怪攻击，
...
- ⚠️ **修改任何 JS/CSS/图片后，必须同步更新 `index.html` 中的 `?v=` 版本号**（当前 = 28），
  否则浏览器缓存会导致改了不生效
...
- 战场布局（v3.10）：战斗页中部 5 区 —— boss区（魔王战）→ 小鬼区（敌方，331 宽）→ 界河区（信息）→
  副将区（=手牌，331 宽）→ 主将区（头像/名字/法宝/血量/防御/卡包/天机）；小鬼与手牌对立，排列 2-3-3-2
  （魔王战小鬼 2+3、手牌 3+2，每排 3 张由 `battleRenderer.HAND_ROW_SIZE=3` 控制）
